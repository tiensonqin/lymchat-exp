(ns lymchat.handlers
  (:require
   ...
   [goog.string :as gs]
   [lymchat.core-async-storage :as s]))
...
(register-handler
 ...
 (fn [db _]
   (s/get-db
    (fn [persistent-db]
      (dispatch [:merge-db persistent-db])))
   db))

(register-handler
 :merge-db
 (fn [db [_ persistent-db]]
   (merge db persistent-db)))
...
(register-handler
 ...
 (fn [db _]
   (when-let [latest-id (:id (first (:mentions db)))]
     (storage/kv-set :mention-latest-id latest-id))
   db))
...
(register-handler
 :set-guide-step
 (fn [db [_ value]]
   (storage/kv-set :guide-step value)
...
(register-handler
 ...
 (fn [db [_ conversation-id]]
   (storage/delete-conversation conversation-id)
   (-> db
...
(register-handler
 ...
 (fn [db [_ user]]
   (let [user (util/wrap-created-at user)]
     (when-not (storage/invite-exists? user)
       (ui/vibrate))

     (try
       (storage/create :invites user)
       (catch js/Error e
         (prn e)))
...
(register-handler
 :set-current-user
 (fn [db [_ value]]
   (storage/kv-set :current-user value)
...
(register-handler
 :set-no-disturb?
 (fn [db [_ value]]
   (storage/kv-set :no-disturb? value)
...
(register-handler
 :set-username-set?
 (fn [db [_ value]]
   (storage/kv-set :username-set? value)
   (assoc db :username-set? value)))

(register-handler
 :set-username-success
 (fn [db [_ value]]
   (storage/kv-set :username-set? true)
   (-> db
...
(register-handler
 ...
 (fn [db [_ channel]]
   (try
     (storage/join-channel (util/uuid->str channel))
     (catch js/Error e
       nil))
                                        ...
                                        (if (or (and (:id message) (nil? (:message_id con)))
                                          ...
                                          (let [m {:message_id (:id message)
                                            ...
                                            (let [new-con (merge con m)]
                                              (storage/update-conversations new-con)
                                              new-con))
                                          con))
                                      con))))))))
...
(register-handler
 ...
 (fn [db [_ current-callee]]
   (assoc db
          ...
          :current-messages (storage/get-conversation-messages (get-in db [:current-user :id]) current-callee 20))))
...
(register-handler
 ...
 (fn [db [_ message]]
   (let [message (storage/wrap-conversation-id message)
...
(register-handler
 ...
 (fn [db [_ message]]
   (let [channel (storage/get-channel-by-id (:channel_id message))]
     (dispatch [:jump-in-channel-conversation channel]))
   db))
   ...
   (let [current-user   (:current-user db)
         ...
         message (-> bare-msg
                     (assoc :id msg-temp-id
                            ...
                            :is_delivered true)
                     (storage/wrap-conversation-id :to_id))]
                      ...
                      (fn [reply]
                        (if (sente/cb-success? reply)
                          (let [new-id (:new-message-id reply)]
                            (storage/delete :messages msg-temp-id)
                            (storage/create :messages (assoc message :id new-id))
   ...
   (let [current-user   (:current-user db)
     ...
     (ws/send-message bare-msg
                      ...
                      (fn [reply]
                        (if (sente/cb-success? reply)
                          (let [new-id (:new-message-id reply)]
                            (storage/delete :group-messages msg-temp-id)
                            (storage/create :group-messages (assoc msg :id new-id))
     ...
     (ws/send-message (assoc bare-msg
                      ...
                      (fn [reply]
                        (if (sente/cb-success? reply)
                          (let [new-id (:new-message-id reply)]
                            (storage/delete :messages msg-temp-id)
                            (storage/create :messages (assoc msg :id new-id)))
     ...
     (doseq [[channel-id messages] col]
       (when (> (count messages) limit)
         (let [message-id (:id (first (take-last limit (sort-by :created_at messages))))]
           (storage/delete-channel-old-messages channel-id message-id)))))
   db))
     ...
     (ws/send-channel-message bare-msg
                              ...
                              (fn [reply]
                                (if (sente/cb-success? reply)
                                  (let [new-id (:new-message-id reply)]
                                    (storage/delete :group-messages msg-temp-id)
                                    (storage/create :group-messages (-> msg
                                                                        (dissoc :channel_name)
                                                                        (assoc :id new-id)))
                                    (dispatch [:update-new-msg-id channel-id msg-temp-id new-id]))
                          ...
                          (ws/send-message (assoc bare-msg
                                           ...
                                           (fn [reply]
                                             (if (sente/cb-success? reply)
                                               (let [new-id (:new-message-id reply)]
                                                 (storage/delete :messages msg-temp-id)
                                                 (storage/create :messages (assoc msg
                                                                                  :id new-id
                                                                                  :body body)))
...
(register-handler
 ...
 (fn [db [_]]
   (assoc db :conversations (storage/get-conversations))))
...
(register-handler
 :jump-in-lym
 (fn [db _]
   (dispatch [:jump-in-channel-conversation storage/lym-channel])
   db))

(register-handler
 :mark-conversation-as-read
 (fn [db [_ id]]
   (storage/update-conversations {:id id
                                  :is_read true})
...
(register-handler
 ...
 (fn [db [_ msg-temp-id]]
   (storage/update :messages {:id msg-temp-id
                              :is_delivered false})
...
(register-handler
 ...
 (fn [db [_ channel-id message-id]]
   (storage/delete :group-messages message-id)
   (update-in db
                                  ...
                                  (doall
                                   (map (fn [c]
                                          (if (and (:is_channel c) (= (:channel_id message) (:to c)))
                                            (do
                                              (storage/create :group-messages message)
                                              (storage/update-conversations (assoc m :id (:id c)))
                                              (merge c m))
                                            c))
                                     col))))))))
