...
(register-sub
 ...
 (fn [db _]
   (reaction
    (util/distinct-by-id (get @db :conversations)))))
...
(register-sub
 ...
 (fn [db _]
   (reaction
    (let [mention-latest-id (storage/kv-get :mention-latest-id)
 ...
 (fn [db _]
   (reaction
    (let [channel-id (get @db :current-channel)
          c (storage/get-channel-members-count channel-id)]
      (if (zero? c)
        1
        c)))))
...
(register-sub
 ...
 (fn [db _]
   (reaction
    (if-let [nav (:nav @db)]
      (let [route (.-routeName (.getCurrentRoute nav))]
        (contains? #{:channel-conversation :conversation} (keyword route)))
      true))))
