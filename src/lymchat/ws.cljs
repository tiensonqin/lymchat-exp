...
(defn request-updated-contacts
  []
  (let [last (storage/kv-get :contact-last-update-at)
        now (tc/now)]
    (when (and
           last
           (tc/after? now
                  ...
                  (fn [reply]
                    (if (sente/cb-success? reply)
                      (when-let [contacts (:updated-contacts reply)]
                        (storage/batch-create :contacts contacts)
                        (storage/kv-set :contact-last-update-at
                                      (tcc/to-date (tc/now))))))))))
...
(defn sync
  ...
  []
  (chsk-send! [:chat/sync {:contacts-ids (storage/get-contacts-ids)}]
              5000
              (fn [reply]
                (when (sente/cb-success? reply)
                  (try
                    (let [{:keys [new-contacts new-invites channels channels-messages]} reply]
                      (storage/batch-create :contacts (util/uuid->str new-contacts))
                      (storage/batch-create :invites (util/uuid->str new-invites))
                      (storage/batch-create :channels (util/uuid->str channels))
                      (let [messages (flatten (vals channels-messages))]
                        (storage/batch-create :group-messages messages))
...
(defmethod push-msg-handler :chat/reply-unread-messages
  ...
  (storage/batch-insert-messages data)
...
(defmethod -event-msg-handler :chsk/handshake
  ...
  (let [[?uid ?csrf-token ?handshake-data] ?data]
    ...
    (when (storage/me)
      (request-unread-messages (storage/get-latest-message-id)))

    (sync)

    (request-updated-contacts)

    (when (not (storage/kv-get :welcome))
      (try
        (storage/kv-set :welcome true)

        (storage/create :channels storage/lym-channel)
        (storage/create-channel-conversation (:id storage/lym-channel)
                                           {:user_id "lymchat"
                                            :name "lymchat"
                                            :username "lymchat"
    ...
    (when (and (not (storage/kv-get :welcome))
               (storage/kv-get :username-set?))
      (storage/kv-set :welcome true)
      (api/channel-join (:id storage/lym-channel) nil nil))
